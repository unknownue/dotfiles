...
function prompt_dir -d "Display the current directory"
  prompt_segment 2C2C2C FFFFFF (basename $PWD) # (prompt_pwd)
end
...
function prompt_status -d "the symbols for a non zero exit status, root and background jobs"
    if [ $RETVAL -ne 0 ]
      prompt_segment 3C3C3C red "✘"
    else
      # if superuser (uid == 0)
      set -l uid (id -u $USER)
      if [ $uid -eq 0 ]
        prompt_segment 3C3C3C yellow "⚡"
      else
        # Jobs display
        if [ (jobs -l | wc -l) -gt 0 ]
          prompt_segment 3C3C3C cyan "⚙"
        else
          prompt_segment 3C3C3C white "λ"
        end
      end
    end
end
